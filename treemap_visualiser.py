...
It is responsible for initializing an instance of TMTree by using a
concrete subclass, rendering it to the user using pygame,
...
from tm_trees import ChessTree, dir_tree_from_nested_tuple, \
    moves_to_nested_dict, example_tree
...
FONT_ROWS = 1  # rows of text we leave room for in display
    ...
    while True:
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            ...
            if event.key in KEY_MAP:
                ...
                sn = selected_node
                selected_node = execute_task_expand_collapse_actions(event,
                                                                       sn)
                execute_task_other_actions(event, hover_node, selected_node)

                execute_task_open_action(event, selected_node)
            else:
...
def execute_task_open_action(event: pygame.event.Event,
...
def execute_task_expand_collapse_actions(event: pygame.event.Event,
...
def execute_task_other_actions(event: pygame.event.Event,
